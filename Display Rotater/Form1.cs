...
namespace Display_Rotater
{
    ...
    public partial class Form1 : Form
    {
        ...
        public void InitializeContextMenu()
        {
            ...
            displayItems = new MenuItem[screens.Length];
            for (int i = 0; i < screens.Length; i++)
            {
                displayItems[i] = new MenuItem(screens[i].DeviceName, SelectDisplay);
                displayItems[i].Checked = selectedScreen.DeviceName == screens[i].DeviceName;
            }

            var horizontalItem = new MenuItem("Horizontal", SetHorizontal);



            var verticalItem = new MenuItem("Vertical", SetVertical);

            var contextMenu = new ContextMenu();
            contextMenu.MenuItems.AddRange(displayItems);
            contextMenu.MenuItems.Add("-");
            contextMenu.MenuItems.Add(horizontalItem);
            contextMenu.MenuItems.Add(verticalItem);
            contextMenu.MenuItems.Add("-");
            contextMenu.MenuItems.Add("Exit", ExitApp);
        ...
        {
            lastMenuLocation = Control.MousePosition;
        }

        private void SelectDisplay(object sender, EventArgs e)
        {
            var itemName = ((MenuItem)sender).Text;
            selectedScreen = Screen.AllScreens.First(s => s.DeviceName == itemName);
        }
        ...
        {
            SetOrientation(Orientations.DEGREES_CW_0);
        }

        private void SetVertical(object sender, EventArgs e)
        {
            SetOrientation(Orientations.DEGREES_CW_270);
        }

        private bool SetOrientation(Orientations Orientation)
        {
            uint DisplayNumber = ((uint)selectedScreen.DeviceName[selectedScreen.DeviceName.Length - 1] - (uint)'0');
